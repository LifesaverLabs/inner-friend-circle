...
export function NayborSOSDialog({
  ...
  const handleCopyMessage = () => {
    ...
    toast.success(t('nayborSOS.toasts.messageCopied'));
  };
  ...
  const handleContactAll = () => {
    ...
    if (toContact.length === 0) {
      toast.error(t('nayborSOS.toasts.noNayborsSelected'));
      return;
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'category' && t('nayborSOS.steps.category')}
            {step === 'compose' && categoryInfo && `${categoryInfo.icon} ${categoryInfo.name}`}
            {step === 'contacts' && t('nayborSOS.steps.contacts')}
          </DialogDescription>
        </DialogHeader>
            ...
            >
              <div className="p-3 rounded-lg bg-tier-naybor/5 border border-tier-naybor/20">
                ...
                <div className="text-xs text-muted-foreground">
                  <p className="font-medium mb-1">{t('nayborSOS.suggestedActions')}</p>
              ...
              <div>
                <label className="text-sm font-medium mb-1.5 block">
                  {t('nayborSOS.addDetails')}
                </label>
                <Textarea
                  placeholder={t('nayborSOS.describePlaceholder')}
                  value={customMessage}
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  aria-label={t('nayborSOS.chooseNayborsAria')}
                >
                  {t('nayborSOS.chooseNaybors')}
            ...
            >
              ...
              {contactedNaybors.size > 0 && (
                <p className="text-xs text-center text-muted-foreground" aria-live="polite">
                  {t('nayborSOS.contacted', { count: contactedNaybors.size })}
                </p>
              )}
            </motion.div>
          )}
